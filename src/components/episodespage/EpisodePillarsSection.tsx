...
import { PillarCard, PillarCardSkeleton } from "@/components/characterspage/PillarsCard";
import {
  useGetPillarsByType,
  useCreateEpisodeRatingPillar,
  useGetUserEpisodeRatingPillars,
} from "@/hooks/use-pillars";
import { useSession } from "@/lib/auth-client";
import { mediaType } from "@/generated/prisma/enums";

interface EpisodePillarsSectionProps {
  episodeId: string;
}
...
export function EpisodePillarsSection({ episodeId }: EpisodePillarsSectionProps) {
  ...
  const {
    ...
    error: pillarsError,
  } = useGetPillarsByType(mediaType.EPISODE);

  const { data: userRatings, isLoading: isLoadingRatings } =
    useGetUserEpisodeRatingPillars(userId, episodeId);

  const createRatingMutation = useCreateEpisodeRatingPillar();
  ...
  const handleQuizComplete = useCallback(
    ...
    [quizState, userId, episodeId, createRatingMutation]
  );
      ...
      <div className="max-w-5xl mx-auto relative">
        ...
        >
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-3 animate-in fade-in slide-in-from-bottom-3 duration-500 delay-100">
            {quizState.mode === "selection" ? "Rate this Episode" : quizState.pillar.name}
          </h2>
...
export default EpisodePillarsSection;
