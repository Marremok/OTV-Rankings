...
export function EpisodeHero({ title, episodeNumber, heroImageUrl, season }: EpisodeHeroProps) {
  ...
  return (
    <section className="relative h-[90vh] w-full flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute top-6 left-6 z-30">
        <BackToButton fallback="/rankings/episodes" fallbackLabel="Rankings" />
      </div>

      {/* Bottom-left content: episode badge + title */}
      <div className="absolute bottom-12 left-8 md:left-12 z-20 max-w-3xl">
        <p className="text-sm text-primary font-bold tracking-widest mb-3 animate-in fade-in slide-in-from-bottom-4 duration-700">
          {seasonLabel}
        </p>
        <h1 className="text-3xl sm:text-5xl md:text-7xl font-bold tracking-tighter text-white drop-shadow-[0_4px_24px_rgba(0,0,0,0.5)] animate-in fade-in slide-in-from-bottom-6 duration-1000 delay-100">
          {title}
        </h1>
      </div>
