...
import {
  useGetPillarsByType,
  useCreateSeasonRatingPillar,
  useGetUserSeasonRatingPillars,
} from "@/hooks/use-pillars";
import { useSession } from "@/lib/auth-client";
import { mediaType } from "@/generated/prisma/enums";

interface SeasonPillarsSectionProps {
  seasonId: string;
}
...
export function SeasonPillarsSection({ seasonId }: SeasonPillarsSectionProps) {
  ...
  const {
    ...
    error: pillarsError,
  } = useGetPillarsByType(mediaType.SEASON);

  const { data: userRatings, isLoading: isLoadingRatings } =
    useGetUserSeasonRatingPillars(userId, seasonId);

  const createRatingMutation = useCreateSeasonRatingPillar();
  ...
  const handleQuizComplete = useCallback(
    ...
    [quizState, userId, seasonId, createRatingMutation]
  );
      ...
      <div className="max-w-5xl mx-auto relative">
        ...
        >
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-3 animate-in fade-in slide-in-from-bottom-3 duration-500 delay-100">
            {quizState.mode === "selection" ? "Rate this Season" : quizState.pillar.name}
          </h2>
...
export default SeasonPillarsSection;
