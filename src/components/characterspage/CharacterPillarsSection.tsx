...
import type { Pillar, QuizAnswer } from "@/components/seriespage";
import { PillarCard, PillarCardSkeleton } from "./PillarsCard";
import {
  useGetPillarsByType,
  useCreateCharacterRatingPillar,
  useGetUserCharacterRatingPillars,
...
interface CharacterPillarsSectionProps {
  characterId: string;
}
...
export function CharacterPillarsSection({ characterId }: CharacterPillarsSectionProps) {
  const { data: session } = useSession();
  const userId = session?.user?.id;

  // Fetch pillars with mediaType CHARACTER from the database
  const {
    data: dbPillars,
    isLoading: isLoadingPillars,
    error: pillarsError,
  } = useGetPillarsByType(mediaType.CHARACTER);

  // Fetch this user's existing character ratings (to show rated / unrated state)
  const { data: userRatings, isLoading: isLoadingRatings } =
    useGetUserCharacterRatingPillars(userId, characterId);

  // Mutation for saving a completed quiz rating
  const createRatingMutation = useCreateCharacterRatingPillar();
  ...
  // Transform DB pillars into the UI Pillar shape
  ...
  // Map existing user ratings for quick lookup (pillarId → score)
  ...
  const handlePillarSelect = useCallback((pillar: Pillar) => {
    ...
    setTimeout(() => {
      setQuizState({ mode: "quiz", pillar, questionIndex: 0, answers: [] });
  ...
  const handleQuizComplete = useCallback(
    async (score: number) => {
      ...
      finalAnswers[quizState.questionIndex] = {
        questionId: currentQuestion.id,
        score,
        weight: currentQuestion.weight,
      };
      ...
      try {
        ...
        console.error("Failed to save character rating:", error);
        toast.error("Failed to save rating", { description: "Please try again." });
      }
    },
    [quizState, userId, characterId, createRatingMutation]
  );
  ...
  const renderContent = () => {
    ...
    if (pillarsError) {
      return (
        <ErrorState message={pillarsError.message || "Failed to load rating categories"} />
      );
    }
    ...
    if (quizState.mode === "quiz") {
      return (
        <QuizQuestion
          ...
          initialValue={quizState.answers[quizState.questionIndex]?.score ?? 0}
    ...
    // Selection mode — render all pillar cards
    return (
      <div className="flex flex-col gap-4">
        {pillars.map((pillar, index) => (
          <PillarCard
            ...
            disabled={!userId || pillar.questions.length === 0}
          />
        ))}
      </div>
    );
      ...
      <div className="max-w-5xl mx-auto relative">
        ...
        >
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-3 animate-in fade-in slide-in-from-bottom-3 duration-500 delay-100">
            {quizState.mode === "selection" ? "Rate this Character" : quizState.pillar.name}
          </h2>
          <p className="text-zinc-500 max-w-md mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 delay-150">
            {quizState.mode === "selection"
              ...
              : `Answer ${quizState.pillar.questions.length} question${quizState.pillar.questions.length === 1 ? "" : "s"} to rate this pillar`}
          </p>
        </div>
        ...
        {/* Back to selection (shown during quiz) */}
        {quizState.mode === "quiz" && (
...
export default CharacterPillarsSection;
