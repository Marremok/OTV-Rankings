...
import { useGetUserRatingsForMultipleSeasons, useGetPillarCount } from "@/hooks/use-pillars"
import { useSession } from "@/lib/auth-client"
import { Card } from "@/components/ui/card"
import { ChevronRight, Layers, Loader2, Star } from "lucide-react"
...
export default function SeasonsRankingPage() {
  ...
  const { data: totalPillarCount = 0 } = useGetPillarCount(mediaType.SEASON)
  ...
  return (
    <div className="relative min-h-screen bg-background text-foreground py-16 px-4 md:px-8 overflow-hidden">
      ...
      <div className="relative max-w-5xl mx-auto">
        ...
        {isError && (
          <p className="text-destructive py-16 text-center">Failed to load seasons.</p>
        )}

        {!isLoading && !isError && seasons.length === 0 && (
          <p className="text-muted-foreground py-16 text-center">No seasons ranked yet.</p>
        )}
              ...
              return (
                ...
                >
                  ...
                  <div className={cn(
                    ...
                    getScoreColor(season.score).neon
                  )} />
                  ...
                  <Link href={detailHref} onClick={saveScroll} className="relative aspect-[2/3] w-28 sm:w-36 md:w-44 shrink-0 overflow-hidden">
                    {season.posterUrl ? (
                      ...
                      <div className="flex h-full w-full items-center justify-center bg-zinc-900">
                        <Layers className="size-14 text-zinc-700" />
                      </div>
                    )}
                  ...
                  <div className="relative flex flex-1 flex-col justify-between p-4 md:p-8 lg:p-10">
                    ...
                    <div className="space-y-5">
                      ...
                      <p className="hidden sm:block max-w-2xl line-clamp-2 text-sm font-light leading-relaxed text-zinc-500 transition-colors duration-500 group-hover:text-zinc-400">
                        {season.description || "Season details coming soon."}
                      </p>
                    </div>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-4 border-t border-white/10 pt-4 md:pt-8">
                      <div className="flex flex-wrap gap-4 md:gap-8 lg:gap-16">
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="flex items-baseline gap-2">
                            <span className={cn(
                              ...
                              getScoreColor(season.score).text
                            )}>
                        ...
                        {/* USER RATING STATUS */}
                        {(() => {
                          const userRatings = userRatingsMap[season.id] || []
